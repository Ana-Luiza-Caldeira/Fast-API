...
from schemas.usuario_schema import UsuarioSchema, UsuarioSchamaArtigos, UsuarioSchemaBase, UsuarioSchemaCreate, UsuarioSchemaUp
...
@router.get('/logado', response_model=List[UsuarioSchemaBase])
...
@router.get('/{usuario_id}', response_model=UsuarioSchamaArtigos, status_code=status.HTTP_200_OK)
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario: UsuarioSchamaArtigos = result.scalars().unique().one_or_none()
        if usuario:
            return usuario
        else:
...
async def delete_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario_del: UsuarioSchamaArtigos = result.scalars().unique().one_or_none()
        if usuario_del:
